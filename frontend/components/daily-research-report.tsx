...
type ZeroDteOptionIntent = { symbol: string; signal_date: string; option_type: "CALL" | "PUT"; direction: "LONG_CALL" | "LONG_PUT"; expiration: string; underlying_price: string; strike: string; max_premium_budget: string; status: string; rationale: string[] };
type ResearchReport = { generated_at: string; candidates: Candidate[]; backtests: Backtest[]; portfolio: Portfolio; options_watchlist: OptionsWatch[]; zero_dte_option_intents: ZeroDteOptionIntent[] };
...
function CandidateGrid({ candidates, generatedAt, optionsWatchlist, zeroDteIntents }: Readonly<{ candidates: Candidate[]; generatedAt: string; optionsWatchlist: OptionsWatch[]; zeroDteIntents: ZeroDteOptionIntent[] }>) {
  return <div className="space-y-5"><SummaryStrip items={[["Paper capital", "$10,000"], ["Candidates", String(candidates.length)], ["0DTE intents", String(zeroDteIntents.length)], ["Options watch", String(optionsWatchlist.length)], ["Generated", generatedAt.slice(0, 10)]]} /><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">{candidates.map((candidate) => <CandidateCard candidate={candidate} key={candidate.symbol} />)}</div><ZeroDteOptionPanel intents={zeroDteIntents} /><OptionsWatchPanel optionsWatchlist={optionsWatchlist} /><p className="text-xs text-terminal-muted">These are next-session paper/research candidates. 0DTE CALL/PUT rows are option-intent plans only; no options orders are executed without an options chain provider.</p></div>;
}
...
function TradeJournal({ backtests, portfolio, zeroDteIntents }: Readonly<{ backtests: Backtest[]; portfolio: Portfolio; zeroDteIntents: ZeroDteOptionIntent[] }>) {
  ...
  return <div className="space-y-4"><SummaryStrip items={[["Ending equity", usd(portfolio.ending_equity)], ["Realized trades", String(portfolio.closed_trades)], ["0DTE intents", String(zeroDteIntents.length)], ["Win rate", pct(portfolio.win_rate)], ["Profit factor", portfolio.profit_factor]]} /><ZeroDteOptionPanel intents={zeroDteIntents} />{trades.length === 0 ? <p className="text-sm text-terminal-muted">No strategy trades were generated from the current real-data report.</p> : <TradeTable trades={trades} />}</div>;
}
...
function AnalyticsGrid({ report }: Readonly<{ report: ResearchReport }>) {
  return <div className="space-y-4"><PortfolioMetricGrid portfolio={report.portfolio} /><EquityVisualization report={report} /><BacktestGrid backtests={report.backtests} portfolio={report.portfolio} /><TradeJournal backtests={report.backtests} portfolio={report.portfolio} zeroDteIntents={report.zero_dte_option_intents} /></div>;
}
...
function ZeroDteOptionPanel({ intents }: Readonly<{ intents: ZeroDteOptionIntent[] }>) {
  if (intents.length === 0) return <p className="text-sm text-terminal-muted">No 0DTE CALL/PUT paper intents are available from the current real-data report.</p>;
  return <div className="rounded-xl border border-sky-400/30 bg-sky-400/5 p-4"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold text-sky-200">0DTE CALL / PUT Paper Intents</h3><span className="text-xs uppercase tracking-[0.2em] text-terminal-muted">No options execution</span></div><p className="mt-2 text-xs text-terminal-muted">These rows translate the latest underlying research signal into same-expiration option intent plans. Premiums, Greeks, bid/ask liquidity, open interest, and fills require a configured options provider before execution.</p><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[920px] text-left text-xs text-terminal-muted"><thead className="bg-black/30 text-terminal-text"><tr>{["Ticker", "Expiry", "Type", "Direction", "Underlying", "Strike", "Max premium", "Status", "Rationale"].map((h) => <th className="px-3 py-2" key={h}>{h}</th>)}</tr></thead><tbody>{intents.map((intent) => <tr className="border-t border-terminal-border" key={`${intent.symbol}-${intent.option_type}-${intent.expiration}`}><td className="px-3 py-2 text-terminal-text">{intent.symbol}</td><td className="px-3 py-2">{intent.expiration}</td><td className={intent.option_type === "CALL" ? "px-3 py-2 text-emerald-300" : "px-3 py-2 text-red-300"}>{intent.option_type}</td><td className="px-3 py-2">{intent.direction}</td><td className="px-3 py-2">{usd(intent.underlying_price)}</td><td className="px-3 py-2">{usd(intent.strike)}</td><td className="px-3 py-2">{usd(intent.max_premium_budget)}</td><td className="px-3 py-2">{intent.status}</td><td className="px-3 py-2">{intent.rationale.join(" ")}</td></tr>)}</tbody></table></div></div>;
}
